...
export function EventCard({ event, index }: EventCardProps) {
    ...
    return (
        <motion.div
            ...
            className="group relative bg-white/80 backdrop-blur-xl rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl border border-white/60 transition-all"
        >
            {getStatusBadge()}
            ...
            <div className={`relative h-48 bg-gradient-to-br ${event.image} overflow-hidden`}>
                ...
                {/* Category Badge */}
                <div className="absolute top-4 left-4 px-3 py-1.5 bg-white/90 backdrop-blur-sm rounded-full text-xs font-bold text-slate-700 shadow-md capitalize">
            ...
            <div className="p-6">
                {/* Title */}
                <h3 className="text-xl font-black text-slate-900 mb-2 line-clamp-1 group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-purple-600 group-hover:to-pink-600 group-hover:bg-clip-text transition-all">
                    {event.title}
                </h3>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="flex -space-x-2">
                                {event.lineup.slice(0, 3).map((artist) => (
                                    <div
                                        key={artist.id}
                                        className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-400 to-pink-400 border-2 border-white shadow-md flex items-center justify-center text-white text-xs font-bold"
                                        title={artist.name}
                                    >
